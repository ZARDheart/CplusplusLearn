...
int main()
{
    ...
    // 并且 icp.getFitnessScore()（均方误差）为0，相关的转换矩阵为单位矩阵。
    cout <<"Maximum Iterations: "<<icp.getMaximumIterations()<<" has conveged: " << icp.hasConverged() << " score:" << icp.getFitnessScore() << endl;
    cout << "matrix:\n" << icp.getFinalTransformation() << endl;
    pcl::transformPointCloud(*input_cloud, *cloud_final, icp.getFinalTransformation());
    // Saving transformed input cloud.
	pcl::io::savePCDFileASCII("../room_scan2_transformed_ICP.pcd", *cloud_final);

    // --6 可视化
    pcl::visualization::PCLVisualizer::Ptr viewer_final(new pcl::visualization::PCLVisualizer("3D Viewer(ICP)"));
