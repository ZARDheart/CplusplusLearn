...
int main()
{
    // --1 双目相机标定
	//读取所有图像
	vector<Mat> imgLs,imgRs;
	string imgLName,imgRName,imagepath="../../opencv/data/calibrate/";
	ifstream finL("../../opencv/data/calibrate/steroCalibDataL.txt");
	ifstream finR("../../opencv/data/calibrate/steroCalibDataR.txt");
	while (getline(finL, imgLName) && getline(finR, imgRName))
	{
		Mat imgL = imread(imagepath+imgLName);
		Mat imgR = imread(imagepath+imgRName);
	...
	vector<vector<Point2f>> imgLsPoints, imgRsPoints;
	getImgsPoints(imgLs, imgLsPoints, board_size);//调用子函数检测棋盘格内角点在图像中坐标
	...
	for (int i = 0; i < imgLs.size(); i++)//进行校正映射
	{
		Mat img1r, img2r;
		...
		imshow("校正后结果"+windowNumber, result);
	}
    waitKey();
